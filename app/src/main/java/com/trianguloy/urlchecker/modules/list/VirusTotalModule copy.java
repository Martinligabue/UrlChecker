...
import com.trianguloy.urlchecker.modules.companions.urlscanioUtility;
...
/**
 * This module uses the urlscan.io api (https://urlscan.io/?/) for url reports
 */
public class urlscan.ioModule extends AModuleData {
    ...
    public AModuleDialog getDialog(MainDialog cntx) {
        return new urlscan.ioDialog(cntx);
    }

    @Override
    public AModuleConfig getConfig(ModulesActivity cntx) {
        return new urlscan.ioConfig(cntx);
    }
}

class urlscan.ioConfig extends AModuleConfig {

    final GenericPref.Str api_key;

    public urlscan.ioConfig(ModulesActivity cntx) {
        super(cntx);
        api_key = urlscan.ioModule.API_PREF(cntx);
    }
    ...
    public int getLayoutId() {
        return R.layout.config_urlscan.io;
    }
...
class urlscan.ioDialog extends AModuleDialog {
    ...
    private urlscan.ioUtility.InternalReponse result = null;

    private final GenericPref.Str api_key;

    public urlscan.ioDialog(MainDialog dialog) {
        super(dialog);
        api_key = urlscan.ioModule.API_PREF(dialog);
    }
    ...
    private void _scanUrl() {
        urlscan.ioUtility.InternalReponse response;
        while (scanning) {
            // asks for the report
            response = urlscan.ioUtility.scanUrl(getUrl(), api_key.get(), getActivity());
     ...
     *
     * @param details if true, the urlscan.io page is opened, if false just a basic dialog with the json
     */
